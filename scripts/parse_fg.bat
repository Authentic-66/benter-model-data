...
echo Parsing FG: %PDF%
echo.
py "%SCRIPTS%brisnet_parser_v2.py" "%PPDIR%\%PDF%" FG
echo.
...
goto :done
:log_picks
copy /y "%SCRIPTS%%PICKS%" "%SCRIPTS%handicap-logs\HANDICAP_FG_%TODAY%.txt" > nul
echo Picks logged: handicap-logs\HANDICAP_FG_%TODAY%.txt
:done
echo.
pause
