@echo off
title ROI Tracker - FG
set "SCRIPTS=%~dp0"
for %%d in ("%SCRIPTS%..") do set "BASE=%%~fd"
set "RESDIR=%BASE%\Fair Grounds\fg-results-2026"

for /f %%d in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd"') do set "TODAY=%%d"
if not exist "%SCRIPTS%roi-logs\" mkdir "%SCRIPTS%roi-logs"
set "LOGFILE=%SCRIPTS%roi-logs\ROI_FG_%TODAY%.txt"
echo ROI Tracker - FG  [%TODAY%] > "%LOGFILE%"

for /f "delims=" %%f in ('dir /b /o-d /a-d "%SCRIPTS%picks_FG_*.txt" 2^>nul') do (
    set "PICKS=%%f"
    goto :gotpicks
)
echo No picks_FG_*.txt found in scripts folder. Run parse_fg.bat first.
echo No picks_FG_*.txt found in scripts folder. Run parse_fg.bat first. >> "%LOGFILE%"
pause & exit /b 1
