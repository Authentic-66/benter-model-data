...
echo.
py "%SCRIPTS%brisnet_parser_v2.py" "%PPDIR%\%PDF%" MVR
echo.
...
goto :done
:log_picks
copy /y "%SCRIPTS%%PICKS%" "%SCRIPTS%handicap-logs\HANDICAP_MVR_%TODAY%.txt" > nul
echo Picks logged: handicap-logs\HANDICAP_MVR_%TODAY%.txt
:done
echo.
pause
