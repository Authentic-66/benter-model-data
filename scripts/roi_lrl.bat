@echo off
title ROI Tracker - LRL
set "SCRIPTS=%~dp0"
for %%d in ("%SCRIPTS%..") do set "BASE=%%~fd"
set "RESDIR=%BASE%\Laurel Park\lrl-results-2026"

for /f %%d in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd"') do set "TODAY=%%d"
if not exist "%SCRIPTS%roi-logs\" mkdir "%SCRIPTS%roi-logs"
set "LOGFILE=%SCRIPTS%roi-logs\ROI_LRL_%TODAY%.txt"
echo ROI Tracker - LRL  [%TODAY%] > "%LOGFILE%"

for /f "delims=" %%f in ('dir /b /o-d /a-d "%SCRIPTS%picks_LRL_*.txt" 2^>nul') do (
    set "PICKS=%%f"
    goto :gotpicks
)
echo No picks_LRL_*.txt found in scripts folder. Run parse_lrl.bat first.
echo No picks_LRL_*.txt found in scripts folder. Run parse_lrl.bat first. >> "%LOGFILE%"
pause & exit /b 1
